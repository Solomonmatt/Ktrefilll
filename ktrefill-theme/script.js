...
const FALLBACK_OPERATORS = {
  GH: [{ id: 'mtn-gh', name: 'MTN Ghana' }, { id: 'airtel-gh', name: 'AirtelTigo' }],
  NG: [{ id: 'mtn-ng', name: 'MTN Nigeria' }, { id: 'airtel-ng', name: 'Airtel Nigeria' }, { id: 'glo-ng', name: 'Glo Nigeria' }],
  US: [{ id: 'att-us', name: 'AT&T' }, { id: 'tmobile-us', name: 'T-Mobile' }, { id: 'verizon-us', name: 'Verizon' }],
  GB: [{ id: 'ee-gb', name: 'EE' }, { id: 'o2-gb', name: 'O2' }, { id: 'three-gb', name: 'Three' }],
  KE: [{ id: 'safaricom-ke', name: 'Safaricom' }, { id: 'airtel-ke', name: 'Airtel Kenya' }],
  ZA: [{ id: 'mtn-za', name: 'MTN South Africa' }, { id: 'vodacom-za', name: 'Vodacom' }]
};
...
function getWalletBalance() {
  ...
  return Math.max(0, baseBalance - totalSpent);
}
...
function updateDashboardSummary() {
  ...
  if (dashboardNodes.balance) dashboardNodes.balance.textContent = `$${getWalletBalance().toFixed(2)}`;
...
async function fetchReloadlyCountries() {
  ...
  if (!stored || !hasReloadlyCredentials(stored)) {
...
async function fetchReloadlyOperators(countryCode) {
  ...
  for (const apiBase of apiBases) {
    ...
    if (response.ok) {
      const data = await response.json();
      return Array.isArray(data) ? data : getFallbackOperatorOptions(countryCode);
    }
  }
...
async function renderCarrierCatalog() {
  ...
  root.innerHTML = '<div class="catalog-loading">Loading live operator offers…</div>';
...
function initSigninForm() {
  ...
  const handleSocialLogin = (provider) => {
    ...
    const email = `user+${provider}@ktrefill.local`;
    const name = `${providerNames[provider] || 'Social'} User`;
    ...
    showStatus(statusBox, `Signed in with ${providerNames[provider] || provider}. Redirecting to your dashboard...`, 'success');
    window.setTimeout(() => {
  ...
  signinForm.addEventListener('submit', (event) => {
    ...
    if (mode === 'signup') {
      ...
      if (existingProfile && existingProfile.email && existingProfile.email.toLowerCase() === formData.email.toLowerCase()) {
    ...
    const auth = authenticateLocalUser(formData.email, formData.password);
    if (!auth.success) {
...
function initDashboard() {
  ...
  const userName = dashboardRoot.querySelector('.dashboard-welcome-name');
  ...
  if (userName) userName.textContent = session.name || session.email.split('@')[0];
  if (userGreeting) userGreeting.textContent = `You last signed in on ${new Date(session.signedInAt || Date.now()).toLocaleString()}. Manage your airtime, gift cards and orders from here.`;
  if (userLastLogin) userLastLogin.textContent = new Date(session.signedInAt || Date.now()).toLocaleString();
  if (profileName) profileName.textContent = session.name || session.email.split('@')[0];
